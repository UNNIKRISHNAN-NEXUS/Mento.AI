...
Uses SBERT (Sentence-Transformers) to generate embeddings and FAISS for fast similarity search.
...
def get_embedding_model():
    """Lazy load SBERT model for embedding generation."""
    global _MODEL_INSTANCE
    if _MODEL_INSTANCE is None:
        from sentence_transformers import SentenceTransformer
        logger.info("Loading SBERT model (all-MiniLM-L6-v2)...")
        # Downloads on first use (~90MB) and caches locally
...
def match_syllabus_to_document(
    ...
    # Normalize for cosine similarity (Inner Product on L2-normalized vectors is Cosine Similarity)
    ...
    # Use full_context (contains Unit > Section > Topic) to capture context
